...
from sdk.research_review_packet import build_research_review_packet_payload
...
def main() -> int:
    ...
    parser.add_argument(
        "--packet-label",
        default="phase3-research-review-packet",
    )
    parser.add_argument("--reviewer-id", default="local-reviewer")
    args = parser.parse_args()

    queue_payload = load_or_build_queue(args)
    ...
    if args.decision_index_json:
        decision_index_payload = json.loads(
            args.decision_index_json.read_text(encoding="utf-8")
        )
    else:
    ...
    packet_payload = build_research_review_packet_payload(
        review_queue_payload=queue_payload,
        decision_payloads=decision_payloads,
...
def load_or_build_queue(args: argparse.Namespace) -> dict[str, object]:
    if args.review_queue_json:
        return json.loads(args.review_queue_json.read_text(encoding="utf-8"))
    if args.bundle_index_json:
        bundle_index_payload = json.loads(args.bundle_index_json.read_text(encoding="utf-8"))
    else:
        if args.bundle_json:
            bundle_payloads = [
                json.loads(path.read_text(encoding="utf-8"))
