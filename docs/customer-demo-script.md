...
- Paper Simulation Controlled Submit UI:
  ...
  - Confirm the UI says `Paper Only`, `ENABLE_LIVE_TRADING=false`, and
    `BROKER_PROVIDER=paper`.
  - Submit only the bounded demo payload generated by the UI. Do not enter broker
    credentials, account IDs, API keys, certificates, or customer financial data.
  - After submit, confirm the UI displays `workflow_run_id`, order ID, final OMS
    status, SQLite persistence, and the paper broker simulation message.
