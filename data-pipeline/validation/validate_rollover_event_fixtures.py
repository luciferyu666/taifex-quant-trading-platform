#!/usr/bin/env python3
"""Validate local Phase 2 rollover event CSV fixtures.
...
from app.domain.market_data import validate_rollover_event_rows  # noqa: E402
...
def default_fixtures() -> list[tuple[Path, bool]]:
    return [
        (FIXTURE_DIR / "rollover_events_valid.csv", True),
        (FIXTURE_DIR / "rollover_events_invalid.csv", False),
    ]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local rollover event CSV fixtures without external data access.",
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def main() -> int:
    ...
    report_payload = {
        "artifact_type": "phase_2_rollover_event_fixture_validation",
        "paper_only": True,
    ...
    print("Local rollover event CSV fixtures validated. No external data was downloaded.")
    return 0
